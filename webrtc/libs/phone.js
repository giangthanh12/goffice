...
// ========================
let hostingPrefex = 'webrtc/';
let wssServer = 'webrtc-hn.worldfone.vn';           // eg: raspberrypi.local
let profileUserID = getDbItem("profileUserID", null);   // Internal reference ID. (DON'T CHANGE THIS!)
let profileUser = 'Gemstech';       // eg: 100
let profileName = 'Test';       // eg: Keyla James
let WebSocketPort = '7779';   // eg: 444 | 4443
let ServerPath = '/ws';         // eg: /ws
// let SipUsername = '101';       // eg: webrtc
// let SipPassword = '12345678';       // eg: webrtc
...
// let hostingPrefex = getDbItem("HostingPrefex", "");
...
$(document).ready(function () {
    // Load Langauge File
    $.getJSON(baseHome+'/'+ hostingPrefex + "lang/en.json", function (data){
        lang = data;
        InitUi();
    });
});
...
function ReceiveCall(session) {
    ...
    // $('#ngat').linkbutton('disable');
    // $('#nghe').linkbutton('enable');
    // $('#tuchoi').linkbutton('enable');
    ...
    if ("Notification" in window) {
        if (Notification.permission === "granted") {
            ...
            var noticeOptions = { body: "Cuộc gọi đến...", icon: getPicture(buddyObj.identity) }
            var inComingCallNotification = new Notification(lang.incoming_call, noticeOptions);
...
function RejectCall(lineNumber) {
    // $('#dlg-call').dialog('close'); //Hải thêm vào
    var lineObj = FindLineByNumber(lineNumber);
    if (lineObj == null) {
...
function wireupAudioSession(lineObj) {
    ...
    session.on('accepted', function (data) {
        // Khi khách hàng nhấc máy
        $('#ketthuc').linkbutton('enable');
        $('#huy').linkbutton('disable');
        //--------------
    ...
    session.on('failed', function (response, cause) {
        ...
        $('#dlg-call').modal('hide'); //Hải thêm vào
    });
...
function DialByLine(type, buddy, numToDial, CallerID){
    if(userAgent == null || userAgent.isRegistered()==false){
        notify_error('Chưa kết nối với tổng đài');
    } else {
...
function AddLineHtml(lineObj){
    ...
    // html += "<button onclick=\"cancelSession('"+ lineObj.LineNumber +"')\" class=hangupButton><i class=\"fa fa-phone\" style=\"transform: rotate(135deg);\"></i> "+ lang.cancel +" xxx</button>";
    // html += "</div>";
    // html += "</div>";
