...
$.ajax({ 
    ...
    success: function (json) {
        ...
        $("#thuc_thu_all").html(formatCurrency(thucthu+''.replace(/[,VNĐ]/g,'')));
...
function loc(){
    ...
    $("#bieudo").remove();
    $('#title_thongke').html('Thống kê doanh từ '+info.time_s+' đến '+info.time_e);

    var table = $(".user-list-table").DataTable();
    table.ajax.url(baseHome + "/baocaodoanhthu/loc_socai?time_s=" + info.time_s + "&time_e=" + info.time_e).load();
    table.draw();
    ...
    $.ajax({ 
        ...
        dataType: "json",
        data: info,
        url: baseHome + "/baocaodoanhthu/loc",
        success: function (json) {
            var donhang = json.donhang;
            var sotien = 0;
            var sono = 0;
