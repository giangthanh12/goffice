...
class thanhtoan_Model extends Model{
    ...
    function get_data_combo($phongban){
        ...
        $query = $this->db->query("SELECT id, name AS text FROM thanhtoan $dieukien ");
    ...
    {
        $result = array();
        $dieukien = " WHERE tinh_trang>0 ORDER BY ngay DESC ";
        $query = $this->db->query("SELECT *,
    ...
    {
        $query = $this->insert("thanhtoan", $data);
        if($query){
    ...
    {
        $query = $this->update("thanhtoan", $data, "id=$id");
        if($query){
            $nhatky = array(
    ...
    {
        $query = $this->db->query("SELECT so_tien,nhan_vien FROM thanhtoan WHERE id = $id ");
        $temp = $query->fetchAll(PDO::FETCH_ASSOC);
        if($temp) {
        ...
        if($temp) {
            $id = $temp[0]['id'];
            $thanhtoanmoi = $temp[0]['tam_ung'] + $sotien;
            $query = $this->update("bangluong", ['tam_ung' => $thanhtoanmoi], "id=$id");
        } 
        return $query;
    }
