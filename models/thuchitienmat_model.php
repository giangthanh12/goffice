<?php
class Thuchitienmat_model extends Model{
    ...
    function addObj($data)
    {
        ...
        if($query){
            ...
            AND tai_khoan = 0 AND ngay_gio < '$ngaygio'
    ...
    function updateChotsodu()
    {
        ...
        AND tai_khoan = 0 order by ngay_gio asc limit 1");
        $temp2 = $query2->fetchAll(PDO::FETCH_ASSOC);
        $soducuoi = 0;
        ...
        $query = $this->db->query("SELECT id,so_tien,loai,so_du FROM socai WHERE  tinh_trang > 0
        AND tai_khoan = 0 order by ngay_gio  asc");
        $rows = $query->fetchAll(PDO::FETCH_ASSOC);
        foreach($rows as $item){
            if ($item['loai'] == 0) {
                $sodu = $soducuoi + $item['so_tien'];
    ...
    function delObj($id,$data)
    {
        ...
        if($query){
            ...
            $query5 = $this->db->query("SELECT ngay_gio FROM socai WHERE  id = $id");
